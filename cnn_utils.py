...
def calculate_fan_in_and_fan_out(shape):
    ...
    if len(shape)<2:
        ...
        # todo calculate fan_in and fan_out (i.e. the number of input/output parameters
...
def xavier(shape, seed=None):
    ...
    top = np.sqrt(6/(n_in + n_out))
    bottom = - top
    w = np.random.uniform(low=bottom, high=top, size=shape)
    return w
...
class SoftMax:
    ...
    def backward(self):
        if self.a.grad is not None:
            sum_term = self.grad * self.value
            self.a.grad += self.grad * self.value - np.sum(sum_term) * self.value
...
class Conv:
    ...
    def __init__(self, input_tensor, kernel, stride=1, padding=0):
        ...
        self.stride = stride
        self.grad = None if kernel.grad is None and input_tensor.grad is None else np.zeros(0, dtype=DATA_TYPE)
        self.value = None
...
class MaxPool:
    ...
    def forward(self):
        ...
        for c in range(in_channels):
            for i in range(output_height):
                for j in range(output_width):
                    subregion = self.input_tensor.value[i*self.stride:i*self.stride+self.kernel_size, j*self.stride:j*self.stride+self.kernel_size, c]
                    self.value[i, j, c] = np.max(subregion)
    ...
    def backward(self):
        ...
        for c in range(in_channels):
            for i in range(int(height / self.stride)):
                for j in range(int(width / self.stride)):
                    window = self.input_tensor.value[i*self.stride:i*self.stride+self.kernel_size,
                                                      j*self.stride:j*self.stride+self.kernel_size,
                                                      c]
                    max_idx = np.unravel_index(np.argmax(window), window.shape)
                    input_grad[i*self.stride+max_idx[0], j*self.stride+max_idx[1], c] = self.grad[i, j, c]
...
class CNN:
    ...
    def init_params_with_xavier(self):
        ...
        # calculate number of channels based on input size
        num_channels = 3
        # calculate number of classes based on dataset
        ...
        param_dict = {
            ...
            "fc3_weight": xavier((84, num_classes)),
    ...
    def build_computational_graph(self):
        ...
        # Conv1
        conv1_out = self.conv_op(input_tensor, self.params["conv1_kernel"])
        conv1_out = self.nn_binary_op(Add,conv1_out,self.params["conv1_bias"])

        # RELU1
        relu1_out = self.nn_unary_op(RELU, conv1_out)

        # MaxPool1
        maxpool1_out = self.maxpool_op(relu1_out)


        # Conv2
        conv2_out = self.conv_op(maxpool1_out, self.params[f"conv2_kernel"])
        conv2_out = self.nn_binary_op(Add,conv2_out,self.params[f"conv2_bias"])
        
        # RELU2
        relu2_out = self.nn_unary_op(RELU, conv2_out)


        # MaxPool2
        maxpool2_out = self.maxpool_op(relu2_out)

        # Flatten
        flatten_out = self.nn_unary_op(Flatten, maxpool2_out)

        # FC1
        fc1_out = self.nn_binary_op(VDot, flatten_out, self.params[f"fc1_weight"])
        fc1_out = self.nn_binary_op(Add, fc1_out, self.params[f"fc1_bias"])

        # RELU3
        relu3_out = self.nn_unary_op(RELU, fc1_out)

        # FC2

        fc2_out = self.nn_binary_op(VDot, relu3_out, self.params[f"fc2_weight"])
        fc2_out = self.nn_binary_op(Add, fc2_out, self.params[f"fc2_bias"])

        # RELU4
        relu4_out = self.nn_unary_op(RELU, fc2_out)

        # FC3

        fc3_out = self.nn_binary_op(VDot, relu4_out, self.params[f"fc3_weight"])
        fc3_out = self.nn_binary_op(Add, fc3_out, self.params[f"fc3_bias"])

        # Softmax
        pred = self.nn_unary_op(SoftMax, fc3_out)
